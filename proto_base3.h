...
public:
	...
	struct vector_field :public field_info {
		FieldType value_type;
	};

	struct map_field :public field_info {
		FieldType key_type;
		FieldType value_type;
	};
private:
	size_t serialize_(void *data, unsigned char *dest, FieldType type, FieldType vale_type = TYPE_ERROR, FieldType key_type = TYPE_ERROR) {
		...
		//case TYPE_VECTOR: {
		//	next += serialize_vector_(data, dest, vale_type);
		//}break;
		default:
			break;
		}
	...
	size_t unserialize_(void *data, const unsigned char *dest, FieldType type, FieldType vale_type = TYPE_ERROR, FieldType key_type = TYPE_ERROR) {
	...
	size_t size_(void *data,FieldType type ) {
		size_t size = 0;
		switch (type)
		{
		case TYPE_UINT32: {
	...
	FieldType get_filed_type() {
		if (typeid(Container) == typeid(vector<T1>))
			...
			return FieldType(sizeof(T1));// TYPE_UINT32;
		else if (is_signed<T1>() && sizeof(T1) <= 4)
			return FieldType(sizeof(T1) + 8); //TYPE_SINT32;
		else if (is_unsigned<T1>() && sizeof(T1) == 8)
			return TYPE_I8;// TYPE_UINT64;
		else if (is_signed<T1>() && sizeof(T1) == 8)
			return TYPE_SI8;// TYPE_SINT64;
		else
			return TYPE_ERROR;
	}
protected:
	...
	size_t serialize_vector_(void *data, unsigned char *dest, FieldType type ) {
		switch (type)
		{
		case TYPE_I1:
		case TYPE_SI1: {
			return serialize_vector2_<int8_t>(data, dest, type);
		}break;
		case TYPE_I2:
		case TYPE_SI2: {
			return serialize_vector2_<int16_t>(data, dest, type);
		}break;
		case TYPE_I4:
		case TYPE_SI4: {
			return serialize_vector2_<int32_t>(data, dest, type);
		}break;
		case TYPE_I8:
		case TYPE_SI8: {
			return serialize_vector2_<int64_t>(data, dest, type);
		}break;
		case TYPE_STRING: {
			return serialize_vector2_<string>(data, dest, type);
		}break;
		case TYPE_FLOAT: {
			return serialize_vector2_<float>(data, dest, type);
		}break;
		case TYPE_DOUBLE: {
			return serialize_vector2_<double>(data, dest, type);
		}break;
		case TYPE_MESSAGE: {
			return serialize_vector2_<proto_base>(data, dest, type);
		}break;
		default:
			break;
		}
	...
	size_t serialize_vector2_(void *data, unsigned char *dest, FieldType type) {
		unsigned char *next = dest;
		Container *a = reinterpret_cast<Container *>(data);
		next += uint32_pack(a->size(), next);
		for (auto &ele : *a) {
			next += serialize_(&ele, dest, type);
		}
		return next - dest;
	}
	...
	void set_vector_field(uid_t id, Contianer *a, bool isrequird = false, bool enable = true) {
		auto &field = fields_[id];
		field.type = TYPE_VECTOR;
		field.val_type = get_filed_type<T>();
		field.address = a;
		field.serialize = [a, this](unsigned char *dest)->size_t {
			unsigned char *next = dest;
			next += uint32_pack(a->size(), next);
			for (auto &ele : *a) {
				next += serialize_(&ele, next, get_filed_type<T>());
			}
			return next - dest;
		};
		...
		field.unserialize = [a, this](const unsigned char *dest)->size_t {
			...
			a->resize(len);
			for (auto &ele : *a) {
				next += unserialize_(&ele, dest, get_filed_type<T>());
			}
			return next - dest;
		};
		...
		field.size = [a, this]()->size_t {
			...
			for (auto &ele : *a) {
				size += size_(&ele, get_filed_type<T>());
			}
			return size;
		};
	}
	...
	void set_map_field(uid_t id, Contianer *a, bool isrequird = false, bool enable = true) {
		...
		field.serialize = [a, this](unsigned char *dest)->size_t {
			...
			next += uint32_pack(a->size(), next);
			for (auto &ele : *a) {
				next += serialize_(&ele, next, get_filed_type<T1>());
				next += serialize_(&ele.second, next, get_filed_type<T2>());
			}
			return next - dest;
		};
		field.unserialize = [a, this](const unsigned char *dest)->size_t {
			...
			for (size_t i = 0; i < len; i++) {
				T1 key;
				next+= unserialize_(&key, dest, get_filed_type<T1>());

				T2 val;
				next += unserialize_(&val, dest, get_filed_type<T2>());
		...
		field.size = [a, this]()->size_t {
			...
			size += uint32_size(a->size());
			for (auto &ele : *a) {
				size += size_((T1*)&ele.first, get_filed_type<T1>());
				size += size_(&ele.second, get_filed_type<T2>());
			}
			return size;
		};
	}
	...
	size_t unserialize_(const unsigned char *val) {
		...
		for (auto &field_i : fields_) {
			...
			if (field.type == TYPE_MAP || field.type == TYPE_VECTOR)
				next += field.unserialize(next);
			else
				next += unserialize_(field.address, next, field.type);
		}
		return next - val;
	}
	...
	size_t serialize_(unsigned char *dest) {
		...
		for (auto &field_i : fields_) {
			...
			if (field.type == TYPE_MAP || field.type == TYPE_VECTOR) {
				...
				next += serialize_(field.address, next, field.type);
			}
		}
		return next - dest;
	}
	...
	size_t get_size() {
		...
		for (auto &field_i : fields_) {
			...
			if (field.type == TYPE_MAP || field.type == TYPE_VECTOR)
				...
				size += size_(field.address, field.type);
		}
		return size;
	}
